    ...
    <div class="drop-area" @drop="handleFileDrop" @dragover="handleDragOver" @dragleave="handleDragLeave">
      ...
      <div class="q-py-md row bg-white justify-start items-center text-grey-9 relative-position">
        <template v-if="!isRecordingAudio">
          ...
          <q-btn
            v-if="$q.screen.width > 500 && (ticketFocado.channel === 'whatsapp' || ticketFocado.channel === 'waba')"
            flat
            dense
...
import { EnviarStickerWaba, EnviarTextoWaba, EnviarArquivoWaba, EnviarBotaoWaba, EnviarArquivoUrlWaba, BuscarTemplates, EnviarTemplateWaba, EnviarListaWaba } from 'src/service/waba'
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      gptEnabled: false,
      loading: false,
      accept: '.txt, .xml, .jpg, .png, .pdf, .doc, .docx, .mp4, .xls, .xlsx, .jpeg, .zip, .ppt, .ogg, .mp3, .pptx, image/*, .mpeg',
