...
const checkTicketFilter = (ticket) => {
  ...
  if(ticket.isGroup){
    ...
    if (filtros.status.length > 0 && !filtros.status.includes(ticket.status)) {
      // console.log('Status do ticket não corresponde aos filtros', ticket.status);
      isValid = false;
    }

    // Verificar se já é um ticket do usuário
    if (ticket?.userId == userId) {
      // console.log('Ticket já pertence ao usuário', ticket.userId);
      isValid = true;
    }
    ...
    if (isQueuesTenantExists && ticket.queueId) {
      const isQueueUser = UserQueues.findIndex((q) => ticket.queueId === q.id);
      if (isQueueUser === -1) {
        // console.log('Usuário não tem acesso à fila', ticket.queueId);
        isValid = false;
      }
    }
    ...
    if (isQueuesTenantExists && filtros?.queuesIds.length) {
      ...
      if (isQueue === -1) {
        // console.log('Filas filtradas não incluem a fila do ticket', ticket.queueId);
        isValid = false;
      }
    }

    // Verificar se é um ticket da carteira do usuário
    if (DirectTicketsToWallets() && (ticket?.contact?.wallets?.length || 0) > 0) {
      const idx = ticket?.contact?.wallets.findIndex((w) => w.id == userId);
      if (idx === -1) {
        // console.log('Ticket não pertence à carteira do usuário', ticket);
        isValid = false;
      } else {
        // console.log('Ticket pertence à carteira do usuário');
        isValid = true;
      }
    }

    // Verificar se o parâmetro para não visualizar tickets atribuídos a outros usuários está ativo
    if (isNotViewAssignedTickets() && (ticket?.userId || userId) !== userId) {
      if (!ticket?.userId) {
        isValid = true;
      }
      console.log('isNotViewAssignedTickets está ativo e o ticket não é do usuário', ticket.userId);
      isValid = false;
    }

    // Verificar se o filtro somente tickets não assinados está ativo
    if (filtros.isNotAssignedUser && ticket.userId) {
      // console.log('isNotAssignedUser ativo, mas o ticket está atribuído', ticket.userId);
      isValid = false;
    } else if (filtros.isNotAssignedUser && !ticket.userId) {
      // console.log('isNotAssignedUser ativo e o ticket não está atribuído');
      isValid = true;
    }

     // Se o ticket não possuir fila definida, checar o filtro permite visualizar tickets sem filas definidas é falso.
  ...
  //   else{
  //     console.log('socket ON: ISGROUP:', ticket.id + ':' + ticket.isGroup)
  //     return true
  //   }
  // }
